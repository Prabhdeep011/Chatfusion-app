...
def draw_page_border_and_watermark(c, width, height, border_thickness, border_color):
    # Draw the page border
    c.setLineWidth(border_thickness)
    c.setStrokeColorRGB(*border_color)
    c.rect(10, 10, width - 20, height - 20)  # Margins of 10 on each side

    # Add watermark
    c.setFont("Helvetica", 8)
    c.setFillColorRGB(0.7, 0.7, 0.7)
    c.drawRightString(width - 10, height - 10, "Generated by ChatFusion")
    ...
    for idx, chat in enumerate(st.session_state['history']):
        ...
        def select_font(text):
            if any('\u0A00' <= char <= '\u0A7F' for char in text):
                c.setFont("NotoSansGurmukhi", 10)
            elif any('\u0900' <= char <= '\u097F' for char in text):
                c.setFont("NotoSansDevanagari", 10)
            else:
                c.setFont("Helvetica", 10)

        # Draw initial page content
        if y < margin + 14:  # Check if we need to start a new page
            c.showPage()  # Start a new page
            y = height - margin  # Reset y position for new page
            draw_page_border_and_watermark(c, width, height, border_thickness, border_color)

        # Draw user prompt with appropriate font
        select_font(prompt_text)
        wrapped_prompt = wrap(prompt_text, width=90)
        for line in wrapped_prompt:
            ...
            c.drawString(10 + margin, y, line)  # Left alignment
            y -= 12

        # Draw bot response
        select_font(response_text)
        wrapped_response = wrap(response_text, width=90)
        ...
        if chat.get('image'):
            ...
            image_width, image_height = 180, 180  # Adjust size as needed
            if y < image_height + margin:  # Check if image fits on the current page
                c.showPage()  # Start a new page
                y = height - margin  # Reset y position for new page
                draw_page_border_and_watermark(c, width, height, border_thickness, border_color)

            c.drawImage(image_path, margin, y - image_height, width=image_width, height=image_height)
            y -= (image_height + 14)  # Move down for the next content
...
if pdf_summarizer_option == "Learn to Pronounce":
    ...
    text = st.sidebar.text_area("Enter the text to convert to voice:",height=20)
