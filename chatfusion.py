...
def generate_pdf():
    ...
    for idx, chat in enumerate(st.session_state['history']):
        ...
        # Draw title on every page
        title = "Conversation with ChatFusion"
        title_font_size = 24
        c.setFont("Helvetica-Bold", title_font_size)
        title_width = c.stringWidth(title, "Helvetica-Bold", title_font_size)
        c.drawString((width - title_width) / 2, height - margin - 10, title)

        # Add watermark
        c.setFont("Helvetica", 8)
        c.setFillColorRGB(0.7, 0.7, 0.7)
        c.drawRightString(width - 8, height - 8, "Generated by ChatFusion")
        ...
        for line in wrapped_prompt:
            ...
            c.drawString(margin + 10, y, line)  # Left alignment
            y -= 12
        ...
        if chat.get('image'):
            ...
            if y < image_height + margin:  # Check if image fits on the current page
                ...
                y = height - margin  # Reset y position for new page
                continue  # Skip to the next iteration to redraw the title
