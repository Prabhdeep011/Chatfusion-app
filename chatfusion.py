...
def generate_pdf():
    ...
    def add_page_heading():
        ...
        c.drawCentredString(width / 2, height - margin, "Conversation with ChatFusion")
        c.setFont("Helvetica", 10)
        c.setFillColorRGB(0.7, 0.7, 0.7)
        c.drawRightString(width - 10, height - 15, "Generated by ChatFusion")
        c.setFillColorRGB(0, 0, 0)
        c.setFont("Helvetica", 12)  # Reset to default font for text content

    add_page_heading()
    y -= 40  # Space below the heading
    ...
    for idx, chat in enumerate(st.session_state['history']):
        ...
        if chat.get('image'):
            ...
            if y - image_height < margin:
                c.showPage()
                add_page_heading()
                y = height - margin - image_height - 40
