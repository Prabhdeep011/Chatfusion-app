...
def generate_pdf():
    ...
    # Adjust margins to avoid text being too close to the edge
    margin = 60  
    ...
    for idx, chat in enumerate(st.session_state['history']):
        ...
        wrapped_prompt = wrap(prompt_text, width=75)  # Adjust width for better wrapping
        for line in wrapped_prompt:
            c.drawString(margin, y, line)
            y -= 14  # Adjust line height for better readability
        ...
        wrapped_response = wrap(response_text, width=75)  # Adjust width for better wrapping
        ...
        if chat.get('image'):
            ...
            image_width, image_height = 150, 150  # Adjusted size for images
            # Adjust image position to fit with margin constraints
        ...
        c.setFillColorRGB(0.7, 0.7, 0.7)  # Light grey color for watermark
        c.drawRightString(right_margin, height - 15, "Generated by ChatFusion")
