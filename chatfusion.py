...
def generate_pdf():
    ...
    margin = 20  # Smaller margin for more content space
    y = height - margin

    # Define border thickness and color
    border_thickness = 1
    border_color = (0.5, 0.5, 0.5)  # Light grey for subtle border

    # Indentation settings for user messages
    user_indent = margin + 40  # User messages will be slightly indented to the right
    ...
    for idx, chat in enumerate(st.session_state['history']):
        ...
        def select_font(text):
            if any('\u0A00' <= char <= '\u0A7F' for char in text):  # Gurmukhi (Punjabi)
                ...
                c.setFont("Helvetica", 10)  # Default font for English
        ...
        # Add watermark at the top-right corner on each page
        c.setFont("Helvetica", 8)
        c.setFillColorRGB(0.7, 0.7, 0.7)  # Light grey for subtle watermark
        ...
        wrapped_prompt = wrap(prompt_text, width=90)  # Wrap width adjusted to maximize content fitting
        for line in wrapped_prompt:
            if y < margin + 14:  # Check if there's enough space for the line
                c.showPage()
                y = height - margin
                # Draw border on new page and add watermark again
                c.setLineWidth(border_thickness)
                c.setStrokeColorRGB(*border_color)
                c.rect(margin/2, margin/2, width - margin, height - margin)
                c.setFont("Helvetica", 8)
        ...
        if chat.get('image'):
            ...
            image_width, image_height = 180, 180  # Adjusted size for balance with text
